...
class Search:
    ...
    #Checks if a course number exists
    def classCheck(self, entry):
        if entry in self.allData:
            return True
        else:
            return False

    #Checks if a major exists
    def majorCheck(self, entry):
    ...
    def classSearch(self, entry):
        ...
        for indiv in self.allData[entry]:
            ...
            if profID in classByProf:
                #Making sure it is a valid easy
                if indiv["easy"] != -1:
    ...
    def majorSearchByClass(self, entry):
        ...
        for key in majorClasses:
            ...
            #Making sure there are valid responses
            if len(classByProf) > 0:
                temp = []
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if re.search(classSearchString, entry):
            entry = entry.upper()
            if searchUtil.classCheck(entry):
                results = searchUtil.classSearch(entry)
                if len(results) == 0:
                    print("Sorry, the data on that course does not have enough replies to draw conclusions from\n")
                else:
